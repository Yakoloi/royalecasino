...
var winnings = "winnings not defined";
...
$(document).ready(function () {
    ...
    var deckObj = {
        ...
        playAgain: function () {
            console.log("play again running")
            //reset everything
            winnings = 0;
            $("#playerScore").html("Player Score: 0");
            database.ref("users/bet").set(0);
           // database.ref("users/chips").set(100);
            $("#bet").html("Bet: 0");
    ...
    var game = {
        ...
        playerChoices: function () {
            ...
            $("#buttonView").append("<button class='playerChoiceButtons' data-choice='bet' id='betButton' type='button' class='btn btn-outline-primary'>Bet</button>");
        ...
        /* updateBet: function () {
             alert("clicked on bet");
             database.ref("users/chips").set(tempChips);
             database.ref("users/bet").set(tempuser.bet + 10);
             $("#bet").html("Bet: " + tempuser.bet);
             $("#playerChips").html("PlayerChips: " + tempuser.chips);
    ...
    $(document).on('click', '#betButton', function () {
        alert("clicked on bet");
        database.ref("users/chips").set(tempuser.chips - 10);
        database.ref("users/bet").set(tempuser.bet + 10);
        $("#bet").html("Bet: " + tempuser.bet);
        $("#playerChips").html("PlayerChips: " + tempuser.chips);
        winnings = parseInt(tempuser.bet) + parseInt(tempuser.bet);
        newChip = tempuser.chips + winnings;
    });
